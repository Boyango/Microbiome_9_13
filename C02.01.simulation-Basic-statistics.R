...
#     lapply(1:length.parameter, function(k) { #k: base params 1:34
#       save_file = paste0("output/result-", model, "-", i, ".", j, ".", k,".rds")
#       #readRDS(paste0("output/R0201sim181201/result.",i,".",j,".",k,".rds"))
#       if(file.exists(save_file)) {readRDS(save_file)} else {tmp}
#       }
#     })
#   })
# }) -> result

# unlist(result) %>% matrix(nrow = 4) %>% t %>% as.data.frame %>% dplyr::filter(V4 == 0)
